...
默认进入「理发大师」营业流程：随机客人、点击推进的漫画对白、明星与动物照片参考、高清作品四向查看、理发情绪、完成评分、重试与下一位。现有 20 位明星和 6 种动物参考任务；每个固定目标从兼容池抽取多种初始造型，包含跨预设和明显加长的变化，多个目标共用初始造型。一轮内订单不重复，重试保留本次组合。初始造型与目标保持明显轮廓差异；结算只按最终造型的外形、长度和走向评分，不操作也按正常几何相似度计算。详见 [营业流程与明星发型评分](docs/营业流程与明星发型评分.zh.md)。照片随包加载，运行时离线。
...
打开 http://127.0.0.1:4179/ 进入营业流程；http://127.0.0.1:4179/?sandbox=1 保留自由实验场景；http://127.0.0.1:4179/expressions.html 为五官表情对比页，http://127.0.0.1:4179/model-review/index.html 为原始角色检查页。
...
- 营业模式按漫画提示进入理发，角落照片可点击查看四向目标，完成后比较作品、看分数并接待下一位。
- 自由实验模式顶部可切换 12 款初始发型；切换会清除当前修剪和梳理结果。
- 剪发：手指或鼠标沿头发连续划动，按投影方向生成斜切面，不需要每刀抬手。
- 剃光：涂抹删除局部发根，头部遮挡的后脑需旋转后操作。
- 梳理：拖动改变头发走向，松手保留造型。吹风：从点击风源朝头中心吹，仅作用于当前可见面的头发；按住持续吹，背面不受影响。
...
截至 2026-09-20：类型、字体检查通过；275 项测试通过 274 项。`tests/project.test.cjs` 中禁止 physics 模块的断言与现有引擎配置不一致，属于已知失败，未通过修改断言掩盖。Creator 构建与微信真机验收尚未完成。
